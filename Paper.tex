...
\begin{equation}
\boldsymbol{\sigma}_mathrm{t}+1} \equiv \Upsilon(\boldsymbol{\sigma}_t, T)
\end{equation}
...
\begin{eqnarray}
\boldsymbol{\sigma}_mathrm{t}+1} & \equiv & \Pi(\boldsymbol{\sigma}_t, B) \\
B & \equiv & (..., (T_0, T_1, ...) ) \\
...
\begin{equation}
R \equiv (R_u, R_b, R_\mathbf{l}, R_mathrm{s}'})
\end{equation}
...
We assert that the status code $R_mathrm{s}'}$ is an integer.
\begin{equation}
R_mathrm{s}'} \in \mathbb{P}
\end{equation}
...
\end{equation}
\begin{equation}
O_a \in \mathbb{B}_{20} \quad \wedge \quad \forall_mathrm{t} \in O_\mathbf{t}}: t \in \mathbb{B}_{32} \quad \wedge \quad O_\mathbf{d} \in \mathbb{B}
\end{equation}

We define the Bloom filter function, $M$, to reduce a log entry into a single 256-byte hash:
\begin{equation}
M(O) \equiv \bigvee_mathrm{t} \in \{O_a\} \cup O_\mathbf{t}} \big( M_{3:2048}(t) \big)
\end{equation}
...
\forall_mathrm{i} \in \{0, 2, 4\}}&:& \mathcal{B}_mathrm{m}(\mathbf{x}, i)}(\mathbf{y}) = 1\\
...
\begin{align}
g_0 \equiv {} & \sum_mathrm{i} \in T_\mathbf{i}, T_\mathbf{d}} \begin{cases} G_{txdatazero} & \text{if} \quad i = 0 \\ G_{txdatanonzero} & \text{otherwise} \end{cases} \\
...
where $\mathtt{\tiny KEC}$ is the Keccak 256-bit hash function, $\mathtt{\tiny RLP}$ is the RLP encoding function, $\mathcal{B}_mathrm{a}..b}(X)$ evaluates to binary value containing the bits of indices in the range $[a, b]$ of the binary data $X$ and $\boldsymbol{\sigma}[x]$ is the address state of $x$ or $\varnothing$ if none exists. Note we use one fewer than the sender's nonce value; we assert that we have incremented the sender account's nonce prior to this call, and so the value used is the sender's nonce at the beginning of the responsible transaction or VM operation.
...
\begin{equation}
\mathbf{a}_1 \
equiv (\boldsymbol{\sigma}_1'[s]_n, \boldsymbol{\sigma}_1'[s]_b - v, \boldsymbol{\sigma}_1'[s]_\mathbf{s}, \boldsymbol{\sigma}_1'[s]_c)
\end{equation}
\begin{equation}
...
\begin{equation}
\lVert B_\mathbf{U} \rVert \leqslant 2 \bigwedge_mathrm{U} \in B_\mathbf{U}} V(U) \; \wedge \; k(U, P(B_H)_H, 6)
\end{equation}
...
\qquad\forall_mathrm{U} \in B_\mathbf{U}}: \\ \nonumber
...
We define $\mathbf{R}[n]_mathrm{s}'}$ in a similar manner.
\begin{equation}
\mathbf{R}[n]_mathrm{s}'} =
...
\mathbb{L} & \equiv & \{ \mathbf{t}: \mathbf{t} = ( \mathbf{t}[0], \mathbf{t}[1], ... ) \; \wedge \; \forall_mathrm{n} < \lVert \mathbf{t} \rVert} \; \mathbf{t}[n] \in \mathbb{T} \} \\
\mathbb{B} & \equiv & \{ \mathbf{b}: \mathbf{b} = ( \mathbf{b}[0], \mathbf{b}[1], ... ) \; \wedge \; \forall_mathrm{n} < \lVert \mathbf{b} \rVert} \; \mathbf{b}[n] \in \mathbb{O} \}
\end{eqnarray}
...
\end{cases} \\
\mathtt{\tiny BE}(x) & \equiv & (b_0, b_1, ...): b_0 \neq 0 \wedge x = \sum_mathrm{n} = 0}^{n < \lVert \mathbf{b} \rVert} b_n \cdot 256^{\lVert \mathbf{b} \rVert - 1 - n} \\
...
\begin{equation}
\forall_mathrm{I} \in \mathfrak{I}} I \equiv (I_0, I_1)
\end{equation}
...
\forall_n \quad \forall_mathrm{i}: i < 2\lVert\mathbf{k}_n\rVert} \quad \mathbf{k}_n'[i] & \equiv &
\begin{cases}
...
\texttt{\small RLP}\Big( \big(\texttt{\small HP}(I_0[i .. (j - 1)], false), n(\mathfrak{I}, j) \big) \Big) & \text{if} \quad i \ne j \quad \text{where} \; j = \arg \max_x : \exists \mathbf{l}: \lVert \mathbf{l} \rVert = x : \forall_mathrm{I} \in \mathfrak{I}}: I_0[0 .. (x - 1)] = \mathbf{l} \\
...
Let $F_mathrm{p}^2}$ be a field $F_p[i]/(i+1)$. We define a set $C_2$ with
\begin{equation}
C_2\equiv\{(X,Y)\in F_mathrm{p}^2}\times F_mathrm{p}^2}\mid Y^2=X^3+3\}\cup\{(0,0)\}
\end{equation}
...
&&&& $\forall_mathrm{i} \in \{ 0 \dots \boldsymbol{\mu}_\mathbf{s}[2] - 1\} } \boldsymbol{\mu}'_\mathbf{m}[\boldsymbol{\mu}_\mathbf{s}[0] + i ] \equiv
...
&&&& $\forall_mathrm{i} \in \{ 0 \dots \boldsymbol{\mu}_\mathbf{s}[3] - 1\} } \boldsymbol{\mu}'_\mathbf{m}[\boldsymbol{\mu}_\mathbf{s}[1] + i ] \equiv
...
\texttt{KEC512}(\mathbf{c}_mathrm{i}-1}) & \text{otherwise}
\end{cases}
\end{equation}
...
\begin{equation}
 \mathbf{m}_c(\mathbf{h}, \mathbf{n}, \mathbf{d}) = E_{compress}(E_{accesses}(\mathbf{d}, \sum_mathrm{i} = 0}^{n_{mix}} \mathbf{s}_h(\mathbf{h}, \mathbf{n}), \mathbf{s}_h(\mathbf{h}, \mathbf{n}), -1), -4)
\end{equation}
